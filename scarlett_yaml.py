...
class Mixer(object):
    ...
    def __init__(self):
        ...
        self._keep_interface = ScalarValue()
        self._extension_unit_switch = ScalarValue()
    ...
    def _parse_control(self, name, num_id):
        ...
        #print("parse control name = ", name,"parse control num_id = ", num_id)
        ...
        elif name.startswith("Scarlett 18i20 USB-Sync"):
        ...
        elif name == "Extension Unit Switch":
             self._extension_unit_switch._value = self._get_boolean(num_id)
             self._extension_unit_switch._num_id = num_id
        elif name == "Keep Interface":
             self._keep_interface._value = self._get_boolean(num_id)
             self._keep_interface._num_id = num_id
        else:
    ...
    def _extract_controls():
        ...
        for line in dump.splitlines():
            #print(" extract line : ",line)
            num_id, _, name = line.split(b",")[:3]
            _, num_id = num_id.split(b"=")
            num_id = int(num_id.decode('ascii'))
            _, name = name.split(b"=")
            name = name.decode('ascii').strip("'")
            #print("extract name = ",name,"num_id =", num_id)
            yield name, num_id
